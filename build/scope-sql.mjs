...
function evaluateScopeNode(node         , ctx                  )          {
  switch (node.node) {
    ...
    case 'in': {
      ...
      return (node.values             ).some((value) => sqliteEqual(field, value));
    }
    case 'isNull': {
      const field = ctx.row[String(node.field)];
    ...
    case 'gte': return (ctx.row[String(node.field)]          ) >= (node.value          );
    case 'lte': return (ctx.row[String(node.field)]          ) <= (node.value          );
    case 'not': return !evaluateScopeNode(node.operand           , ctx);
    case 'and': return (node.operands             ).every((operand) => evaluateScopeNode(operand           , ctx));
...
function scopeParamValue(node         , ctx                  )          {
