...
export async function authorizeRow(
  ...
  principal           ,
  preRow                                 = null,
  { allowDeletedAnchor = false, authorization }                                                                         = {},
)                            {
  ...
  if (allowDeletedAnchor) {
    ...
    const materialized = anchorRow ? entity.deserializeRow({ ...(anchorRow                           ) }) : undefined;
...
async function admitEntityRow(
  ...
  if (!authorization) return mayRow(entity                           , verb, row, principal);
...
export async function dispatchCrud({ entity, verb, fieldName, db, principal, params, body, actionId: requestedActionId, app, res,
  sendJson, committedEventHeaders, authorization }                     )                {
  if (!db) {
  ...
  if (verb === 'read') {
    ...
    const auth = await authorizeRow(app               , entity, 'read', params.id, principal, null, { authorization: authz });
    if (auth.status) {
  ...
  if (verb === 'update') {
    const kernel = app?.kernel;
    if (!kernel) return void sendFailure(sendJson, res, failure('internal', 'Internal error.'));
    const auth = await authorizeRow(app               , entity, 'update', params.id, principal, null, { authorization: authz });
    if (auth.status) {
      const denied = auth.status === 404
        ? failure('not-found', 'not found')
        : failure('denied', 'forbidden');
  ...
  if (verb === 'remove') {
    const kernel = app?.kernel;
    if (!kernel) return void sendFailure(sendJson, res, failure('internal', 'Internal error.'));
    const auth = await authorizeRow(app               , entity, 'remove', params.id, principal, null, { authorization: authz });
    if (auth.status) {
      const denied = auth.status === 404
        ? failure('not-found', 'not found')
        : failure('denied', 'forbidden');
