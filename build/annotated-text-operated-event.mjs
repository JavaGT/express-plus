...
function preflightV15Payload(raw                         , operation                         , entity        , field        )       {
  ...
  let prerequisites = 0;
  const visit = (value         , key = '', underText = false)       => {
...
const CREATE_ANNOTATION_KEYS = ['family', 'fields', 'id', 'protectedTargetIds']         ;

function parseV15Range(value         , entity        , field        )                                           {
...
function parseV15Ranges(value         , entity        , field        )                                                      {
...
function parseV15CreateAnnotation(value         , entity        , field        )                             {
  if (!isPlainObject(value) || !exactKeys(value, CREATE_ANNOTATION_KEYS)) invalidEnvelope(entity, field, 15);
  const { id, family, fields, protectedTargetIds } = value
  ...
  const targets = protectedTargetIds            ;
...
function parseV15Transition(value         , entity        , field        , seenIds             )                       {
  if (!isPlainObject(value) || typeof value.kind !== 'string') invalidEnvelope(entity, field, 15);
...
function parseV15Operation(operation                         , entity        , field        )                                                                                                 {
  ...
  const transitions = Object.freeze(
    (operation.transitions                      ).map((entry) => parseV15Transition(entry, entity, field, seenIds)),
  );
  return Object.freeze({
    kind: 'region.edit',
    from: operation.from          ,
...
function parseV16Operation(operation                         , entity        , field        )                                                                                                 {
  const keys = [
    'affectedIds',
    'afterDigest',
    'beforeDigest',
...
const V16_EMPTIED_KEYS = ['annotationId', 'disposition']         ;
const V16_DISPOSITION_KEYS = ['family', 'kind', 'lastRange', 'savedQuote']         ;

function parseV16Emptied(value         , entity        , field        )                             {
  ...
  return Object.freeze(value.map((entry) => {
    ...
    return Object.freeze({
      ...
      disposition: Object.freeze({
        ...
        lastRange: disposition.lastRange                                    ,
      }),
    });
  }));
}

const V16_ORPHAN_KEYS = ['lastRange', 'savedQuote']         ;
const V16_PREREQUISITE_KEYS = ['entity', 'id']         ;
const V16_MEMBERSHIP_KEYS = ['end', 'ordinal', 'start']         ;

function parseV16Endpoint(value         , entity        , field        )          {
...
function parseV16WitnessImage(rawImage         , entity        , field        , seenIds             )                        {
...
function parseV16WitnessSide(value         , entity        , field        )                                   {
  if (!Array.isArray(value)) invalidEnvelope(entity, field, 16);
  const seenIds = new Set        ();
  let previous = '';
 ...
 */
export function parseStoredV16OperatedEvent(eventDataText        , context                                   )                      {
  const { entity, field } = context;
  ...
  try {
    ...
    throw new Error(`${entity}.${field}.operated v16 stored envelope is not canonical: ${(error         ).message}`);
  }
  ...
  return normalizeOperatedEvent(parsed                        , context)                       ;
}
  ...
  const parseValue = (depth        )          => {
    ...
    if (ch === '{') {
      ...
      for (;;) {
        ...
        const key = JSON.parse(text.slice(index, text.indexOf('"', index + 1) + 1))          ;
    ...
    const literalMatch         = /^(true|false|null|-?\d+(\.\d+)?([eE][+-]?\d+)?)/.exec(rest)?.[0] ?? '';
...
export function constructV16RegionEvent(plan            )                                                                                                                                                                                 {
  ...
  const eventDataText = serializeV16OperatedEvent(event                        );
 ...
 */
export function serializeV16OperatedEvent(event                      )         {
  if ((event                         ).version !== 16) {
 ...
 * every check passes.
 */
function canonicalV16Json(value         , maxBytes        , label        )         {
  const out           = [];
  let bytes = 0;
