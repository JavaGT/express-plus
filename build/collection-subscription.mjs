...
export class CollectionSubscriptionBackpressureError extends Error {
  constructor(message        ) { super(message); this.name = 'CollectionSubscriptionBackpressureError'; }
}
...
function isCompiled(rule                                             )                                   {
  return 'sql' in rule && 'params' in rule;
}

export function createCollectionSubscription({ db, entity, principal, rule, mayVerb, authorization = null, deliver, maxPendingDeliveries = 16 }                               )                         {
  ...
  async function admittedRows()                                                                         {
    ...
    for (const value of raw.slice(0, compiled.boundedResultPolicy.limit)) {
      ...
      const allowed = authorization
        ? (await authorization.admit({ category: 'entity', verb: 'read', operation: 'read', principal, entity: entity         , row: hydrated, resourceId: String(hydrated.id ?? '') })).admitted
        : await mayRow(entity         , 'read', hydrated, principal, mayVerb         );
      if (!allowed) continue;
...
export function createCollectionSubscriptionRegistry(maxSubscriptionsPerConnection = 32)                                                                                                                                                                                     {
  const subscriptions = new WeakMap                                     ();
  return {
    add(connection, subscription) {
      const current = subscriptions.get(connection) ?? new Set                        ();
