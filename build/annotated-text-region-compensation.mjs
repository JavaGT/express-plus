...
import { scalarCount } from './annotated-text.mjs';
import {
  materializeText,
...
import { sha256Utf8 } from './annotated-text-region-limits.mjs';
import {
...
} from './annotated-text-region-plan.mjs';
import {
  constructV16RegionEvent,
  parseStoredV16OperatedEvent,

} from './annotated-text-operated-event.mjs';
import { ValidationError } from './field-strategy.mjs';
import { getAnnotatedTextCompiledMetadata } from './annotated-text-field.mjs';
import { privateFactFromReceipt, parseCompoundApplicationTransition,                                   } from './compound-contribution-fact.mjs';
...
export function compensationActor(entity        , field        , documentId        , principalId                    , scope        , actionId        )         {
...
function annotationDeclarationsForLoad(compiledMeta     , descriptor     ) {
  return compiledMeta.annotationHandles
    ? Object.values(compiledMeta.annotationHandles).map((meta     ) => ({
...
function buildRegionDeclarations(compiledMeta     , descriptor     )                      {
  return Object.values(compiledMeta.annotationHandles ?? {}).map((meta     ) => {
...
export function loadRegionCompensationContext(
  ...
  const stored                                                                     = annotations.map((image) => {
    const annConfig      = (descriptor.annotations                                                 )?.find((a) => a.annotationName === image.family);
    const meta      = compiledMeta.annotationHandles?.[image.family];
    return Object.freeze({
      id: image.id,
      family: image.family,
      fields: image.fields                           ,
...
export function targetInsertFromEvent(event                     )                      {
...
function relativeRangesOf(image                 , deletedText        )                                   {
...
function coveredIdsOf(annotations                                                                               , family                      , from        , to        )           {
  const ids           = [];
...
export function readMoveCompensationTarget(
  db          ,
  { scope, entity, field, rootActionId, targetActionId }                                                                                                ,
)                         {
  const receipt = db.prepare(
    'SELECT committedAt FROM _ActionReceipt WHERE scope = :scope AND actionId = :actionId',
  ).get({ scope, actionId: targetActionId })                                       ;
  if (!receipt) throw new TypeError('history move target receipt is missing');
  const envelope = privateFactFromReceipt(db, { scope, actionId: targetActionId, committedAt: receipt.committedAt })                                           ;
  ...
  const originReceipt = db.prepare(
    'SELECT committedAt FROM _ActionReceipt WHERE scope = :scope AND actionId = :actionId',
  ).get({ scope, actionId: rootActionId })                                       ;
  ...
  ).get({ scope, actionId: targetActionId, eventType: `${entity}.${field}.operated` })                                     ;

  let event                             = null;
  if (row) {
    try {
      event = parseStoredV16OperatedEvent(row.eventData          , { entity, field });
    } catch {
      event = null;
    }
  }

  const contribution = (envelope.contributions[0]                                                                       ) ?? null;
