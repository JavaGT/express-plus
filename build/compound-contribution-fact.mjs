...
function compoundMismatch(field        )            {
...
export function canonicalTransitionEqual(left                               , right                               )          {
...
import { parseDeleteFact, isDeleteFact } from './annotated-text-delete-history.mjs';
 ...
 */
export function constructCompoundOriginEnvelope                           ({
  application,
  contributions,
}
 ...
 */
export function constructCompoundCompensationEnvelope                           ({
  application,
  contributions,
  linkage,
}
...
function assertEnvelopeSize(envelope         , where        )       {
  let serialized        ;
  try {
...
export function parseCompoundApplicationTransition(raw         , where        )                                {
 ...
 * fail closed.
 */
export function parseCompoundApplicationTransitionInput(raw         )                                     {
...
function validateApplicationTransitionPayload(before         , after         , where        )       {
 ...
 */
export function compoundKindOf(value         )                                                                         {
 ...
 */
export function parseCompoundContributionFact(raw         )                               {
  ...
  const linkage                              = deepFreeze({
    rootActionId: linkageRaw.rootActionId          ,
    targetActionId: linkageRaw.targetActionId          ,
    direction: linkageRaw.direction                   ,
    outcome: linkageRaw.outcome                      ,
  });
  validateLinkage(linkage);
  return deepFreeze({ version, kind, application, contributions: Object.freeze(contributions), linkage });
}

export function isCompoundContributionFact(value         )                                        {
  try {
 ...
 * (scope#992 rev 4).
 */
export function applicationPrivateFactView(canonicalFact                                   )                                    {
  ...
  return deepFreeze(parsed.application)                                                ;
}
 ...
 */
export function privateFactFromReceipt(db          , receipt                                                          )                                    {
  ...
  if (kind !== null) return parseCompoundContributionFact(fact)                                                ;
