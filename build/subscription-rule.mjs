...
function assertData(value         , label        )       {
...
function compileFilter(filter         , declaredFields             , params                         , next              )         {
  ...
  const name = next();
  params[name] = value;
  const operator = ({ eq: '=', ne: '!=', gt: '>', gte: '>=', lt: '<', lte: '<=' }         )[op];
...
export function compileSubscriptionRule(input         , entity                  )                           {
  ...
  if (!rule.boundedResultPolicy || typeof rule.boundedResultPolicy !== 'object') fail('boundedResultPolicy is required.');
  ...
  const filters = [rule.parent, rule.anchor, ...(rule.filters ?? [])].filter((filter)                               => filter !== undefined);
  if (!Array.isArray(rule.filters ?? []) || filters.length > 32) fail('filters must contain at most 32 entries.');
  const where = filters.map((filter) => compileFilter(filter, declaredFields, params, next));
  const selected = new Set(['id', ...select]);
  const order = [...sort, ...(sort.some((entry) => entry.field === 'id') ? [] : [{ field: 'id', direction: 'asc'          }])];
  const sql = `SELECT ${[...selected].map((field) => `t0.${quote(field)}`).join(', ')} FROM ${quote(entity.name)} AS t0${where.length ? ` WHERE ${where.join(' AND ')}` : ''} ORDER BY ${order.map((entry) => `t0.${quote(entry.field)} ${entry.direction === 'desc' ? 'DESC' : 'ASC'}`).join(', ')} LIMIT :ruleLimit`;
  params.ruleLimit = limit + 1;
