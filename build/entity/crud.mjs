 ...
 */
function captureAnnotationUpdateImage(db     , { prefix, documentId, annotationsDeclarations, annotationId }                                                                                                                                                           )                               {
...
function normalizeAnnotationUpdateImage(image                       ) {
  return {
...
function annotationUpdateImagesEqual(left                       , right                       )          {
 ...
 */
function liveStateMatchesAnnotationUpdateImage(db     , options                                                                                                                                                                                            )          {
 ...
 */
function ordinalsByEncounterOrder(ranges                                                                       ) {
...
function annotatedUpdateCompensation({ name, fieldName, prefix, db, scope, payload, sourceFact, descriptor }
  ...
  const step                                         = direction === 'undo' ? sourceFact.contribution : sourceFact.redo;
  ...
  const noop                                                                                           = { events: [], privateFact: { version: 2, kind: 'annotated-text.compensation', documentId: payload.id, linkage: { rootActionId: payload.history.rootActionId, targetActionId: payload.history.targetActionId, direction, outcome: 'noop' } }, historyOutcome: 'noop' };
        ...
        const originFact = command.edit.kind === 'text.insert'
          ...
          : command.edit.kind === 'annotation.update'
            ? (() => {
              ...
              const after = {
                fields: { ...(events[0].data.facts.annotation.fields                           ) },
                protectedTargetIds: [...captured.protectedTargetIds],
                ranges: ordinalsByEncounterOrder(events[0].data.facts.ranges                                                                 ),
              };
      ...
      for (const [actionName, action] of Object.entries(annotationDeclaration.actions ?? {})                        ) {
        ...
        const threadHandler = async ({ payload, db, scope, principal, actionId }     ) => {
          ...
          for (const [fieldKey, fieldDescriptor] of Object.entries(annotationDeclaration.fields ?? {})) {
            ...
            const rangeRows = annotationRangeRows(db, prefix, payload.id);
           const ranges = rangeRows.map((range     ) => ({ annotationId: range.annotation_id, start: JSON.parse(range.start_point), end: JSON.parse(range.end_point) }));
           const plan = planTextRangeApply({ documentId: payload.id, structureVersion: state.structure_version, family, annotation: { id: annotationId, family: annotationDeclaration.annotationName, fields: annotationFields }       , from: { offset: from, affinity: 'right' }, to: { offset: to, affinity: 'right' }, ranges, actorId: principal?.id ?? '' });
          const handle = eventHandles.native(name, fieldName, 'operated');
