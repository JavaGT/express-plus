...
function compileOutput(entity                , output         , ancestors = new Set                ())                 {
  ...
  for (const [key, value] of Object.entries(object.shape)) {
    ...
    if (node?.kind === 'select') {
      ...
      hasSelect = true;
      for (const field of node.fields                      ) scalar(entity, field);
      entries.push(Object.freeze({ key, kind: 'select', fields: node.fields                      }));
      continue;
    }
    if (node?.kind === 'user') {
      const descriptor = entity.fields[node.via          ];
      ...
      entries.push(Object.freeze({ key, kind: 'user', fk: node.via           }));
      continue;
    }
    ...
    const selectNode = (node.select ?? node.output)                                                                    ;
    const nested = (node.include ?? node.output)                                         ;
    ...
    if (required !== undefined) {
      ...
      const childRef = child.fields[declared.childRef          ];
      ...
      const relatedTarget = typeof childRef.target === 'string' ? null : (childRef.target                                                          );
      ...
      const parentRef = related.fields[declared.via          ];
...
function compileTombstones(declaration                             , resolveEntity               )                       {
  ...
  const entityId = entity.fields[declared.entityId          ];
  ...
  if (declared.scopeId === undefined) {
    ...
    const scopeId = entity.fields[declared.scopeId          ];
    ...
    const sameNamedOwner = target.fields[declared.scopeId          ];
  ...
  return Object.freeze({ target, entity, entityId: declared.entityId          , scopeId: declared.scopeId                 , scopeTarget, targetScopeId, terminalScope, kind: declared.kindField          , state: declared.state          , kindValue: declared.kindValue, hidden: Object.freeze([...(declared.hidden            )]) });
}
...
export function compileSnapshots(declarations         , resolveEntity               , db                  = null)                                   {
  ...
  const internalEntities = new Set        (
    declarations.flatMap((declaration) => {
      const rule = (declaration                                                  )?.tombstones                                                                                        ;
      return [rule?.entity?.name, rule?.terminalScope?.name].filter((name)                 => typeof name === 'string');
    }),
  );
  ...
  for (const declaration of declarations) {
    ...
    compiled.set(declaredAnchor.name, Object.freeze({ anchor, output, tombstone: tombstones })                                  );
  }
...
async function admitSnapshotRow(adapter                                         , entity                , row         , principal         , mayVerb                 )                   {
  if (adapter) {
  ...
  function project(_entity                , node              , branch                )                                 {
    ...
    for (const entry of branch.entries) {
      ...
      for (const child of node.children.get(entry) ?? []) {
        const projected = project(entry.entity, child, entry.nested ?? { entity: entry.entity, entries: entry.selected ? [Object.freeze({ key: 'select', kind: 'select'         , fields: entry.selected })] : [] });
