...
function canonicalStoredAnnotationImage(image                       )                        {
  return {
    id: image.id,
    family: image.family,
...
function parseStoredAnnotationImage(value         , where        , seenIds             )                        {
  if (!isPlainObject(value)) {
  ...
  const memberships                          = [];
  for (const [index, rawMembership] of (record.memberships             ).entries()) {
    if (!isPlainObject(rawMembership) || !exactKeys(rawMembership, ['end', 'ordinal', 'start']) || (rawMembership                           ).ordinal !== index) {
      fail(`${where}.memberships must have dense ordinals`);
    }
    try {
      memberships.push({
        ordinal: index,
        start: assertStructuralEndpoint((rawMembership                           ).start),
        end: assertStructuralEndpoint((rawMembership                           ).end),
      });
    } catch (error) {
      fail(`${where}.memberships[${index}] is invalid: ${(error         ).message}`);
    }
  }
  if (record.orphan !== null) {
    if (!isPlainObject(record.orphan) || !exactKeys(record.orphan, ['lastRange', 'savedQuote']) || typeof (record.orphan                           ).savedQuote !== 'string') {
      fail(`${where}.orphan is invalid`);
    }
    const lastRange = (record.orphan                           ).lastRange;
  ...
  if (Object.hasOwn(record, 'rangeOffsets')) {
    ...
    rangeOffsets = (record.rangeOffsets             ).map((raw, index) => {
      if (
        !isPlainObject(raw) ||
        !exactKeys(raw, ['end', 'ordinal', 'start']) ||
        (raw                           ).ordinal !== index ||
        !isSafeNonNegativeInt((raw                           ).start) ||
        !isSafeNonNegativeInt((raw                           ).end) ||
        ((raw                           ).end          ) <= ((raw                           ).start          )
      ) {
        fail(`${where}.rangeOffsets must contain dense forward ranges`);
      }
      const entry = raw                           ;
      return Object.freeze({ ordinal: index, start: entry.start          , end: entry.end           });
    });
  ...
  const candidate                        = {
    ...
    protectedTargetIds: targets            ,
    memberships,
    prerequisites,
    orphan: record.orphan === null ? null : (
      {
        savedQuote: (record.orphan                           ).savedQuote          ,
        lastRange: (record.orphan                           ).lastRange                                    ,
      }
    ),
 ...
 */
export function parseAnnotationTransition(value         )                       {
  ...
  return deepFreeze({
    before: Object.freeze(parseSide(value.before             , 'annotationTransition.before')),
    after: Object.freeze(parseSide(value.after             , 'annotationTransition.after')),
  });
}
