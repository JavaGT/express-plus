...
export function createBlobLifecycle({ blobs, entities, declaredBlobFields = [] }
  ...
  async function finalizeReplacementGenerations(db          )                  {
    if (!blobs) return 0;
    const replaced = db.prepare('SELECT replacedBy FROM BlobStore WHERE status = ? AND replacedBy IS NOT NULL').all('replaced')                                 ;
    let finalized = 0;
