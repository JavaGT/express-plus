...
> file remains as the *evidence* grounding those decisions and the live eng-review
> specs (cited by section below).
...
scope's central design rule (ADR-0001, restated in ADR-0002 §5) is the
**deletion test**:
...
Apply it to every primitive in the foundation — `app.doc`, the grant compiler,
the effect engine, the live spine. If a primitive does not let us delete
something, it is ceremony.

scope's own worked example that **passes**: `defineManagedResource`, a CRUD
generator that deletes a triplicated payload schema (action shape written three
times — domain, payload, wire) by deriving the wire schema from the domain
schema. It concentrates: three definitions become one. Its server half
(`authorize` + `handler`) stays hand-written, because that half *genuinely
...
scope's hardest invariant, worth importing verbatim:
...
- **One reconciliation path.** The client's `ingest` is the *only* place an event
  ...
  the authoritative `ingest` resolves it. A dual reconciliation path is the
  classic source of "the two clients disagree" bugs; scope structurally forbids
  it.

Both are load-bearing invariants, not preferences: if there is exactly one path,
the developer cannot wire a second one wrong.
...
brands these with unique symbols so an Action is *not assignable* to an Event —
a structural hole TypeScript would otherwise leave open. The event reducer is
**non-optional**: an event type with no reducer is a compile error. If Action and
Event are the same type, a developer can hand an unverified request to a reducer
and the framework cannot tell. Brand them.
...
2. **resolve scope** — a *pure* function `(payload) => scopeRef`, no I/O (§2.4).
3. **authorize** — the grant runs (§2.5).
...
'volatile'` field on actions. Persistence is gated by whether a persistence
module is *wired*: the gate is `action.persistence != null`. The class of an
action (durable, ephemeral, volatile) is **emergent from which seams it
engages**, not a label it carries. You reach a capability by engaging its slot,
not by setting a flag the framework then interprets. A WhatsApp-style chat on
scope's core expresses all three classes from one action primitive, differing
only in engaged seams:
...
Build them as *one action primitive with engaged seams*, not three mechanisms —
"subtract before you add" made concrete: one primitive, three emergent classes,
zero class field.
...
Two anti-foot-gun consequences:

- **Unauthorized requests never open a write transaction.** The scope is known
...
reused, never restated inline. scope's inline-ownership mess is the strongest
evidence that the model is worth its complexity.
...
correctly seen as a duplicate. This closes the double-apply corruption a
separate echo path would open.
...
Undo is *inverse mutations through the one pipeline*, plus client-local preimage
restore for immediacy. Build the preimage capture (§2.2 step 4) and the
...
each *independently durable*: a projection failure never rolls back the committed
...
- **Out-of-band effects** (from scope) — **projections** over the committed log.
  A webhook fires *after* commit, from a projection consumer; its failure is
  retried independently and never rolls back the origin.
...
framework owns the live stream, the journal, the reducers, undo, gap recovery,
and cross-tab sync. The page does not know it is doing realtime — it reads a
reactive snapshot and re-renders. This is "declaration absorbs imperative
wiring" verified against a shipped app.
...
- **The read resync endpoint authorizes at a lower bar than dispatch.** Reading
  ...
  engine, different capability — "no second auth path" with the capability split
...
The findings above now live in the canonical docs (this is the "absorbed" state):

- **AGENTS.md** — the structural invariants scope validates: the deletion test as
  the abstraction gate; one reconciliation path (`ingest` is the only place an
  event becomes state); out-of-band effects are projections over the committed
  log; persistence is opt-in by engaged seam, not a class field; pipeline variants
  are named wholes, not orthogonal boolean flags.
- **SPEC.md** — the vocabulary the live-sync core needs (§6–§9): **action** /
  **event** (branded, distinct), **the pipeline** and its **stages**, **sequence
  cursor**, **ingest**, **preimage**, **projection**, **snapshot** /
  **snapshot sequence**.
- **DECISIONLOG.md** — the out-of-band-effects resolution (§2.8): in-transaction
  effects stay the `{ mutate, with }` primitive; out-of-band effects (webhooks,
  emails, external HTTP) are projections over the committed log, fanned out
  post-commit, independently durable, never rolling back the origin.
...
- **A no-op marker that flips a gate.** The `persistent` module does nothing but
  its presence flips persistence on (§2.3). Make the engaged seam do real work.
...
- **Inconsistent event naming.** One scope event uses a colon (`file:uploaded`)
