...
// "perceived instant" budget. Timing assertions WARN below the budget and fail
// only well above it so slow CI machines do not produce red noise.

const FOLD_PAYLOAD_BUDGET = 128 * 1024;
const SERVER_BUILD_WARN_MS = 20;
const SERVER_BUILD_FAIL_MS = 400;
...
test('client fold apply on a large document stays inside the typing budget (warn over threshold)', async (t) => {
  ...
  const session = createAnnotatedTextHttpSession({
    ...
    onFoldApplied: (fold, elapsedMs) => foldTimings.push(elapsedMs),
  });
