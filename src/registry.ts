...
import { fieldHandle } from './scope-sql.ts';
import {
  PRINCIPAL_ID_PARAM,
  PRINCIPAL_ID_TOKEN,
...
interface CheckRuntime {
  db: DbHandle;
  entityOf: (target: unknown) => unknown;
}

interface CheckRunContext {
  entity: Record<string, unknown>;
  principal: { id: string | null; attributes: Readonly<Record<string, unknown>> };
  runtime: CheckRuntime;
}

interface CheckEntry {
  harvest: (() => unknown) | undefined;
  run: (ctx: CheckRunContext) => boolean;
}

export type CheckRegistry = Readonly<Record<string, CheckEntry>>;

export interface CheckRegistryOptions {
  fields?: Record<string, unknown>;
  declaredChecks?: Record<string, (ctx: unknown) => boolean>;
  entityName?: string;
}

export function buildCheckRegistry({ fields = {}, declaredChecks = {}, entityName }: CheckRegistryOptions): CheckRegistry {
  const registry: Record<string, CheckEntry> = {};

  // ---- Source 1: ref-role fields ----
  for (const [fieldName, descriptor] of Object.entries(fields)) {
    const field = descriptor as { type?: unknown; role?: unknown } | null | undefined;
    if (field?.type === 'ref' && field.role) {
      registry[String(field.role)] = {
  ...
  const compileSelfHandle: Record<string, unknown> = {};
  if (entityName) {
    for (const [fName, desc] of Object.entries(fields)) {
      compileSelfHandle[fName] = fieldHandle(
        fName,
        desc as FieldDescriptor,
        entityName,
        (target: unknown) => typeof target === 'object' ? target : null,
      );
    }
  }
  ...
  for (const [checkName, fn] of Object.entries(declaredChecks)) {
    ...
    const run = ({ entity: row, principal, runtime }: CheckRunContext): boolean => {
      const runtimeSelf: Record<string, unknown> = {};
      if (entityName) {
        for (const [fName, desc] of Object.entries(fields)) {
          const descriptor = desc as { kind?: unknown; type?: unknown; role?: unknown; cells?: unknown } | null | undefined;
          ...
          // a non-link principal, mirroring the harvest FALSE + SQL NULL bind).
            ...
            const handle: Record<string, { is: (v: unknown) => boolean }> = {};
            for (const [cellName] of Object.entries((descriptor as { cells?: Record<string, unknown> }).cells as Record<string, unknown>)) {
              handle[cellName] = {
                is: (v: unknown) => v == null ? false : (row[fName] as Record<string, unknown> | undefined)?.[cellName] === v,
              };
            }
          ...
          else if (descriptor?.kind === 'value' && descriptor.type === 'ref' && !descriptor.role) {
  ...
  for (const [fieldName, descriptor] of Object.entries(fields)) {
    const field = descriptor as { kind?: unknown; type?: unknown; roles?: unknown[] } | null | undefined;
    if (field?.kind === 'store' && field.type === 'map' && Array.isArray(field.roles) && field.roles.length > 0) {
      const table = membershipTable(entityName as string, fieldName);
      const ownerCol = membershipOwnerCol(entityName as string);

      for (const roleName of field.roles) {
        ...
        if (registry[String(roleName)]) continue;

        registry[String(roleName)] = {
...
function membershipHandle(
  ...
  return {
    ...
    get: (memberId: unknown): Record<string, unknown> | undefined => {
      const ownerId = ownerIdOf();
...
function makeRuntimeRefHandle({ fieldName, descriptor, row, runtime }: {
  ...
  const targetReference = (descriptor as { target?: unknown }).target;
  const targetName = typeof targetReference === 'string'
    ? targetReference
    : (targetReference as { name?: unknown } | undefined)?.name;
  const target = targetReference
    ? (runtime.entityOf(targetReference) as { name?: string; fields?: Record<string, unknown> } | null | undefined)
    : null;
  const refId = row[fieldName];
  if (!target?.fields) return refId;
  const targetFields = target.fields;
  const targetTable = (target.name ?? targetName) as string;

  const mapEntries: Array<[string, ReturnType<typeof membershipHandle>]> = [];
  const mapHandles: Record<string, ReturnType<typeof membershipHandle>> = {};
  for (const [targetFieldName, targetDescriptor] of Object.entries(targetFields)) {
    const td = targetDescriptor as { kind?: unknown; type?: unknown } | undefined;
    if (td?.kind === 'store' && td.type === 'map') {
      const table = membershipTable(targetTable, targetFieldName);
      const ownerCol = membershipOwnerCol(targetTable);
  ...
  const handle: Record<string, unknown> = { id: refId, ...mapHandles };

  handle.then = (resolve: (value: unknown) => void, reject: (reason?: unknown) => void): void => {
    try {
      if (refId == null) {
        const result: Record<string, unknown> = { id: refId };
      ...
      const result: Record<string, unknown> = { id: refId };
      if (targetRow) {
        for (const [k] of Object.entries(targetFields)) {
