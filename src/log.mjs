...
function formatHuman(level, channel, message, ctx, at) {
...
export function createLog({ level = 'info', channels = {}, format = 'json', output = process.stderr } = {}) {
  ...
  function emit(level, channel, message, ctx = {}) {
    if (!shouldLog(channel, level)) return;

    if (typeof output === 'function') {
      output(level, channel, message, ctx);
      return;
    }

    if (format === 'json') {
      const entry = { level, channel, at: isoNow(), msg: message };
      if (ctx && Object.keys(ctx).length > 0) {
        const safeCtx = {};
        for (const [k, v] of Object.entries(ctx)) {
      ...
      output.write(formatHuman(level, channel, message, ctx, isoNow()) + '\n', 'utf8');
    }
  }
