...
import { listen as serveListen } from './serve.ts';
import {
  maintenanceDefaults,
  startApplication,
  validateMaintenanceOptions,
} from './application-runtime.ts';
import { buildReadMirrorDescription, wrapDriver } from './driver.ts';
import {
  openMemoryAdapter,
  openSqliteAdapter,
...
import { serveStatic, type ServeStaticOptions } from './views.ts';
...
import { validateExactSchema } from './schema-exact-validation.ts';
import { createSchemaReport } from './schema-report.ts';
...
function schemaObjectKey(object: ReturnType<typeof observedSchemaObjects>[number]): string {
...
export function router(options: any = {}) {
  ...
  surface.resolveFor = async (entityOf: any) => {
...
interface BoundDbAdapter {
  open(): Promise<OpenedDatabase>;
  readMirror(): ReadMirrorDescription;
  isManagedPath?(p: string): boolean;
  // The owned directory root (file mode) or null (memory) — REQUIRED. The
  // sqlite adapter exposes it so the blob-root guard resolves before the
  // deferred open (S6/A2). An adapter that leaves it undefined cannot be
  // classified file-vs-memory, so workbench FAILS CLOSED at construction
  // instead of silently handing a file-backed database the in-memory fake
  // byte store (review #93 finding 1).
...
export default function workbench({
  ...
  let pendingDbAdapter: BoundDbAdapter | null = null;
  ...
  try {
    if (typeof db === 'string') {
      ...
      pendingDbAdapter = db;
      const adapterRoot = (db as BoundDbAdapter).root;
    ...
    let schemaBeforeLifecycle: Set<string> | null = null;
    ...
    const app: any = makeMountable({ entityOf: runtime.entityOf });
    ...
    if (pendingDbAdapter) {
      ...
      app.resolveRoutes = (...args: unknown[]) => installPendingDb!().then(() => originalResolveRoutes(...args));
    ...
    (workbench as any).router = router;
    ...
    let latestSchemaReport: ReturnType<typeof createSchemaReport> | null = null;
    ...
    app.prepareSchema = async () => withLog(app.log, async () => {
      ...
      if (!app.schemaReady) {
        app.schemaReady = (async () => {
          ...
          registryLocked = true;
          const schemaTables = new Map<string, any>((schema?.tables ?? []).map((table: any) => [table.name.toLowerCase(), table]));
          ...
          for (const entity of app.entities.values()) {
            for (const [fieldName, field] of Object.entries(entity.fields ?? {}) as Array<[string, any]>) {
              ...
              if (targetSchema) {
                const id = targetSchema.columns.find((column: any) => column.name.toLowerCase() === 'id');
    ...
    app.static = (prefix: any, dir: any, options: ServeStaticOptions = {}) => app.use(prefix, serveStatic(dir, {
      ...options,
    ...
    app.auth = function auth(options: any = {}) {
      if (!app.db) {
