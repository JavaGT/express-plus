...
import { BACKUP_FORMAT_VERSION, type BackupBinnedGeneration, type BackupManifest } from '../backup.ts';
import type { BlobRecycleSeam } from '../blob-store.ts';

export const RECYCLE_FORMAT_VERSION = 1 as const;
...
export type RecycleDeletion = Readonly<{
...
export type RecycleBlobSeam = {
  resolveBackupBlobName(generation: string, backupBlobsDir: string): string;
};
...
export type RecycleBinnedBackup = Readonly<{
  readonly backupId: string;
  readonly generations: readonly string[];
}>;

export type RecycleFailedBackup = Readonly<{
  readonly backupId: string;
  readonly generations: readonly string[];
  readonly error: string;
}>;

export type RecycleBinResult = Readonly<{
  readonly ok: boolean;
  readonly binned: readonly RecycleBinnedBackup[];
  readonly failed: readonly RecycleFailedBackup[];
}>;

export type RecyclePurgeTarget = Readonly<{ readonly backupId?: string; readonly generation?: string }>;

export type RecycleRestoreTarget = Readonly<{ readonly backupId: string; readonly generation: string }>;

export type RecyclePurgeResult = Readonly<{
  readonly removed: number;
  readonly entries: readonly RecycleBinEntry[];
}>;

export type RecycleRestoreResult =
  | Readonly<{ ok: true; backupId: string; generation: string; entry: RecycleBinEntry }>
  | Readonly<{ ok: false; error: string }>;

export type RecycleManagerOptions = {
  // The adapter-owned directory: backups/ and recycle/ live under it. Pass the
  // same root the backup manager is constructed over (source.root) so binning
  // re-marks the SAME backups the manager created.
  readonly root: string;
  // The generation→file resolution seam (see RecycleBlobSeam). Optional in
  // type so a manager that only lists/restores/purges can be built without it,
  // but REQUIRED for bin(): binning a backup that references blob bytes
  // without a seam refuses fail-closed.
  readonly blobs?: RecycleBlobSeam | null;
  // The recovery period in days before `purge()` destroys binned content.
  // Defaults to DEFAULT_RECYCLE_RETENTION_DAYS (7); configurable, never
  // hardcoded.
  readonly retentionDays?: number;
  readonly now?: () => Date;
};

export type RecycleManager = {
  // The single deletion path: moves every deleted generation's bytes out of
  // every retained backup that references it into recycle/ and re-marks those
  // backups. Identical whether the trigger was privacy erasure or ordinary
  // deletion. Fails closed per backup — a backup whose bytes could not be
  // binned is reported in `failed` with its manifest untouched, never cleaned.
  bin(deletion: RecycleDeletion): Promise<RecycleBinResult>;
  // User-accessible listing (directory + API): every binned entry an operator
  // can restore or purge.
  list(): readonly RecycleBinEntry[];
  // Move one binned generation's bytes back into its origin backup's blobs/
  // dir and un-mark the backup manifest. Fails closed when the origin backup
  // is gone, the destination would be overwritten, or the bytes are missing.
  restore(target: RecycleRestoreTarget): Promise<RecycleRestoreResult>;
  // Destroy binned content. With no target: the expiry sweep (entries binned
  // longer than `retentionDays`). With `{ backupId }` and/or `{ generation }`:
  // force-delete-earlier, regardless of age. Fail closed: a removal that
  // cannot complete rejects instead of reporting success.
  purge(target?: RecyclePurgeTarget): Promise<RecyclePurgeResult>;
  readonly retentionDays: number;
  // The absolute path of the bin directory (operator-visible, excluded from
  // static serving and blob-root acceptance by the S1/A2 managed-path guard).
  readonly root: string;
};
 ...
 */
export function recycleManagerBinSeam(manager: Pick<RecycleManager, 'bin'>): BlobRecycleSeam {
  return {
    bin: async (deletion: { generations: readonly string[] }) => {
...
type RecycleEntryRecord = {
  readonly formatVersion: typeof RECYCLE_FORMAT_VERSION;
  readonly backupId: string;
  readonly generation: string;
  readonly name: string;
  readonly size: number;
...
export function createRecycleManager(options: RecycleManagerOptions): RecycleManager {
  ...
  function entryDir(backupId: string, generation: string): string {
  ...
  type ManifestRead =
    ...
    | { readonly ok: false; readonly kind: 'unreadable' | 'incomplete'; readonly reason: string };

  function readManifestState(dir: string): ManifestRead {
    let raw: string;
    try {
  ...
  function remarkManifest(manifest: BackupManifest, binned: readonly BackupBinnedGeneration[]): BackupManifest {
    if (binned.length === 0) {
      const next: BackupManifest & { binnedGenerations?: readonly BackupBinnedGeneration[] } = { ...manifest };
  ...
  function readEntry(dir: string, backupId: string, generation: string): RecycleBinEntry | null {
    let record: Partial<RecycleEntryRecord>;
    try {
      record = JSON.parse(readFileSync(path.join(dir, 'entry.json'), 'utf8')) as Partial<RecycleEntryRecord>;
    } catch (err) {
  ...
  function listEntries(): RecycleBinEntry[] {
  ...
  function rollbackBin(
    ...
    moved: readonly { generation: string; name: string }[],
  ): string {
  ...
  function binBackup(
  ...
  ): { ok: true; generations: readonly string[] } | { ok: false; generations: readonly string[]; error: string } {
    ...
    const moved: Array<{ generation: string; name: string; size: number; binnedAt: string }> = [];
    try {
      ...
      // half-cleaned.
      const resolved: Array<{ generation: string; name: string; size: number }> = [];
      ...
      for (const { generation, name, size } of resolved) {
        ...
        const record: RecycleEntryRecord = {
    ...
    const records: BackupBinnedGeneration[] = [
  ...
  async function bin(deletion: RecycleDeletion): Promise<RecycleBinResult> {
    ...
    const binned: RecycleBinnedBackup[] = [];
    const failed: RecycleFailedBackup[] = [];
  ...
  function list(): readonly RecycleBinEntry[] {
    return listEntries();
  }

  async function restore(target: RecycleRestoreTarget): Promise<RecycleRestoreResult> {
  ...
  async function purge(target?: RecyclePurgeTarget): Promise<RecyclePurgeResult> {
  ...
  function markPurged(entry: RecycleBinEntry): void {
...
function validateRetentionDays(retentionDays: number | undefined): number {
