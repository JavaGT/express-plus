...
import type { FailureCategory } from './outcome.ts';

export type NoHistoryReceiptOutcome = 'committed' | 'rejected';
...
// the receipt.
export type NoHistoryReceiptErrorClassification = FailureCategory;

export interface NoHistoryReceipt {
  scope: string;
  actionId: string;
  resourceKey: string;
  committedRevision: number;
  outcome: NoHistoryReceiptOutcome;
  actorType: string | null;
  actorId: string | null;
  safeErrorClassification: NoHistoryReceiptErrorClassification | null;
  committedAt: string;
}

export interface NoHistoryReceiptInput {
  scope: string;
  actionId: string;
  resourceKey: string;
  committedRevision: number;
...
export function insertNoHistoryReceipt(db: DbHandle, input: NoHistoryReceiptInput) {
...
export function noHistoryReceiptFor(db: DbHandle, scope: string, actionId: string): NoHistoryReceipt | undefined {
  const row = prepareCached(
    db,
    'SELECT * FROM _NoHistoryReceipt WHERE scope = :scope AND actionId = :actionId',
  ).get({ scope, actionId });
  if (!row) return undefined;
  return row as unknown as NoHistoryReceipt;
}
