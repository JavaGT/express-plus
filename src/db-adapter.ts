...
type DbAdapterPathKey = 'filename' | 'path' | 'file';
...
export type DbAdapterOptions = Readonly<Record<string, unknown>> & {
  readonly [Key in DbAdapterPathKey]?: never;
};
...
export type DbAdapterConfig<
  Options extends DbAdapterOptions = DbAdapterOptions,
> = {
...
export type DbCapabilityFlag = Exclude<keyof DbCapabilities, 'encryption'>;
...
export type ReadMirrorDescription = {
  readonly kind: 'read-mirror';
  readonly mode: 'read-only';
  readonly readOnly: true;
  readonly connectionString: string;
  readonly options?: DbAdapterOptions;
};
...
export interface DbAdapter {
  open(config: DbAdapterConfig): Promise<OpenedDatabase>;
...
export function capabilitiesOf(
  flags: Partial<Record<DbCapabilityFlag, boolean>> = {},
): DbCapabilities {
  return {
