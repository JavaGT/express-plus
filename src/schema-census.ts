...
import {
  ...
  frameworkObjects,
  type FrameworkOwnedObject,
} from './schema-table-census.ts';
import type { OwnedObjectSet } from './framework-table-names.ts';

export type ObjectKind = 'table' | 'index' | 'trigger' | 'virtual-table';
export type OwnerKind = 'framework' | 'entity' | 'schema' | 'plugin' | 'sqlite-artifact';

// One ownership claim in the census.
export interface CensusEntry {
  readonly kind: OwnerKind;
  // 'framework', the owning entity name, the owning schema name, or the owning
  // plugin id. For sqlite-artifact entries this is the owning plugin id.
  readonly owner: string;
  readonly objectKind: ObjectKind | 'shadow-table';
...
export interface FrameworkDeclaration {
  ...
  readonly virtualTables?: readonly string[];
}
...
export interface EntityDeclaration {
  ...
  readonly indexes?: readonly { fields: readonly string[] }[];
}
...
export interface SchemaForeignKeyDeclaration {
...
export interface SchemaTableDeclaration {
  readonly name: string;
  readonly columns: readonly SchemaColumnDeclaration[];
  readonly primaryKey?: readonly string[];
  readonly foreignKeys?: readonly SchemaForeignKeyDeclaration[];
  readonly indexes?: readonly { name: string }[];
  readonly triggers?: readonly { name: string }[];
}

export interface SchemaVirtualTableDeclaration {
  readonly name: string;
  readonly ownerPluginId: string;
  readonly shadowTables?: readonly string[];
}

export interface SchemaMigrationDeclaration {
  readonly namespace: string;
  readonly name: string;
  readonly version: number;
}

export interface SchemaExternalTableDeclaration {
  readonly name: string;
  readonly columns: readonly string[];
}
...
export interface PluginDeclaration {
  readonly id: string;
  readonly ownedObjects?: readonly { kind: ObjectKind; name: string }[];
}
...
export interface ObservedObject {
  readonly type: 'table' | 'index' | 'trigger' | 'virtual-table';
  readonly name: string;
}
...
export interface CensusInput {
  readonly framework?: FrameworkDeclaration;
  readonly schemaDeclarations?: readonly SchemaDeclaration[];
  readonly entities?: readonly EntityDeclaration[];
  readonly plugins?: readonly PluginDeclaration[];
  readonly observed?: readonly ObservedObject[];
  // The census is pure — no database handle is ever accepted. This field is a
  // compile-time poison pill: passing a DbHandle-shaped argument is a type
  // error, and buildOwnershipCensus never reads it.
  readonly db?: never;
}
...
export function censusKey(objectKind: ObjectKind | 'shadow-table', name: string): string {
...
interface ClaimResult {
  readonly entry: CensusEntry | null;
  readonly error: CensusError | null;
}
...
function isTriggerLike(kind: ObjectKind | 'shadow-table'): boolean {
  return kind === 'trigger';
}
...
class ClaimBook {
  readonly census = new Map<string, CensusEntry>();
  private readonly relation = new Map<string, CensusEntry>();
  private readonly triggers = new Map<string, CensusEntry>();
  ...
  claim(
    objectKind: ObjectKind | 'shadow-table',
    name: string,
    ownerKind: OwnerKind,
    owner: string,
    options: { readonly conflictCode?: string; readonly allowSameOwner?: boolean } = {},
  ): ClaimResult {
    ...
    const entry: CensusEntry = Object.freeze({ kind: ownerKind, owner, objectKind, name });
    this.census.set(key, entry);
    namespace.set(folded(name), entry);
    return { entry, error: null };
  }

  ownerOfRelation(name: string): CensusEntry | undefined {
...
function entityMainColumns(entity: EntityDeclaration): ColumnInfo[] {
  const columns: ColumnInfo[] = [{ name: 'id', type: 'TEXT', primaryKey: true }];
  for (const [name, descriptor] of Object.entries(entity.fields ?? {})) {
    const field = descriptor as Readonly<Record<string, unknown>>;
    if (field.kind === 'computed' && field.mode === 'pull') continue;
    if (field.kind === 'struct') {
      for (const cell of Object.keys((field.cells ?? {}) as Record<string, unknown>)) {
...
export function buildOwnershipCensus(input: CensusInput): CensusResult {
  ...
  const reserved = new Map<string, CensusEntry>(); // folded name -> framework entry
  const authEntityOf = new Map<string, string>(); // folded object name -> auth entity name
  const claimFramework = (objectKind: ObjectKind, name: string, fromAuthEntity?: string): void => {
  ...
  const entityObjects = new Map<string, OwnedObjectSet>();

  const isAuthOwnedBySameEntity = (name: string, entity: EntityDeclaration): boolean =>
    authEntityOf.get(folded(name)) === folded(entity.name);

  const refuseReserved = (objectKind: ObjectKind, name: string, claimerKind: OwnerKind, owner: string): boolean => {
  ...
  for (const entity of entities) {
    ...
    const claims: Array<[ObjectKind, readonly string[]]> = [
  ...
  const schemaTablesByEntityName = new Map<string, { schemaName: string; table: SchemaTableDeclaration }>();
  const columnModel = new Map<string, TableInfo>();
  ...
  const entitySideTableNames = new Set<string>();
  ...
  const columnInfoOf = (tableName: string, columnName: string): ColumnInfo | undefined =>
    columnModel.get(folded(tableName))?.columns.find((column) => folded(column.name) === folded(columnName));

  const isIntegerPrimaryKey = (tableName: string, columnName: string): boolean => {
  ...
  const validateSchemaForeignKeys = (schema: SchemaDeclaration): void => {
    for (const table of schema.tables ?? []) {
  ...
  for (const entity of entities) {
    for (const [fieldName, descriptor] of Object.entries(entity.fields ?? {})) {
      ...
      const targetName = (typeof targetValue === 'string' ? undefined : (targetValue as { name?: string } | null | undefined)?.name)
  ...
  // namespace.
  const namespaceOwner = new Map<string, { schema: string; ordinal: number }>();
...
export type Observation =
  | { readonly kind: 'owned'; readonly owner: CensusEntry }
  | { readonly kind: 'undeclared'; readonly object: ObservedObject };
...
export type { FrameworkOwnedObject, OwnedObjectSet };
