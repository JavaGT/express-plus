...
export interface StorageEnvelopePolicy {
  /** Compress a payload only when its UTF-8 byte length is >= this. */
  minBytes: number;
}
...
const storageEnvelopePolicies = new WeakMap<object, StorageEnvelopePolicy>();
...
export function attachStorageEnvelopePolicy(db: DbHandle, policy: StorageEnvelopePolicy | null): void {
  if (policy === null) {
    storageEnvelopePolicies.delete(db as object);
    return;
  }
  storageEnvelopePolicies.set(db as object, policy);
}
...
export function storageEnvelopePolicyFor(db: DbHandle): StorageEnvelopePolicy | undefined {
  return storageEnvelopePolicies.get(db as object);
}
...
export function isStorageEnvelope(value: unknown): value is string {
...
export function encodeStorageEnvelope(plain: string): string {
...
export function decodeStorageEnvelope(envelope: string): string {
...
export function unwrapStoredPayloadText(value: unknown): string {
  return isStorageEnvelope(value) ? decodeStorageEnvelope(value) : value as string;
}
...
export function maybeEnvelopeStoredPayload(db: DbHandle, text: string): string {
