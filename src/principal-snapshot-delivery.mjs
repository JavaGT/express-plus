...
export function validatePrincipalSnapshotDeclarations(declarations, schema) {
...
function resolution(declarations, principal, scope) {
  let parsed;
  try {
...
export function createPrincipalSnapshotDelivery({ db, declarations }) {
  ...
  function pairedSnapshot(resolved) {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      const before = revisionFor(db, resolved.declaration, resolved.principal);
      const snapshot = project(db, resolved.declaration, resolved.principal);
      const after = revisionFor(db, resolved.declaration, resolved.principal);
  ...
  async function drain(id) {
    ...
    try {
      while (sub.active) {
        sub.dirty = false;
        const revision = revisionFor(db, sub.declaration, sub.principal);
  ...
  return Object.freeze({
    ...
    async catchup({ principal, scope, after }) {
      ...
      const revision = revisionFor(db, resolved.declaration, resolved.principal);
    ...
    async subscribe({ principal, scope, after, signal, deliver, revoke }) {
      ...
      const id = nextId++;
      const sub = { ...resolved, scope, cursor: after, deliver, revoke, signal, active: true, pending: false, dirty: false };
      sub.abort = () => remove(id);
      subs.set(id, sub);
