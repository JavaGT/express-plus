...
export interface DeclaredRoutingFact {
  readonly declaration?: string;
  readonly branch?: string;
  readonly entity?: string;
  readonly id?: string;
  readonly scope?: string;
  readonly reason?: 'create' | 'update' | 'remove' | 'reparent' | 'order';
  readonly invalidating?: boolean;
}
...
export function routeCompositeEvent(db: DbHandle, plans: ReadonlyMap<string, AnchorPatchPlan>, event: RouterEvent, evidence: RowEvidence): CompositeChangeInput[] {
  ...
  for (const plan of plans.values()) {
    ...
    const memberId = typeof (event as { data?: { id?: unknown } }).data?.id === 'string' ? ((event as unknown as { data: { id: string } }).data.id) : null;
 ...
 */
export function readCompositeChangesSince(db: DbHandle, scope: string, after: number, limit = 500): StoredCompositeChange[] {
  ...
  ).all({ scope, after, limit }) as Array<Record<string, unknown>>;
