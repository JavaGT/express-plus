...
function validateWith(withValue: unknown, context: string): WithPayload {
  ...
  if (typeof withValue === 'function') {
    ...
    return withValue as (...args: unknown[]) => unknown;
  }
  if (typeof withValue === 'object' && !Array.isArray(withValue)) return withValue as Record<string, unknown>;
...
interface ScheduleDescriptorBase {
...
interface ScheduleAtDescriptor extends ScheduleDescriptorBase {
  kind: 'schedule.at';
}

interface ScheduleAfterDescriptor extends ScheduleDescriptorBase {
  kind: 'schedule.after';
  delay: number;
}
...
export function simulate({ hz, step, when }: SimulateOptions = {}): SimulateDescriptor {
  ...
  return Object.freeze({ kind: 'simulate', hz, step: step as (...args: unknown[]) => unknown, when: (when ?? undefined) as ((...args: unknown[]) => unknown) | undefined });
}
