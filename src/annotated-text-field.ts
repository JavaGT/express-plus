...
export function annotationEntityAction(name: string, options: any = {}): any {
  ...
  const compiled = {
    ...
    annotationHandles: Object.freeze(
      Object.fromEntries([...annotationNames].map((n: any) => {
        ...
        const actionEntries = (annConfig?.actions ?? []).map((action: any) => [action.actionName, Object.freeze({
        ...
        const actionHandles: any[] = actionEntries.map((entry: any[]) => entry[1]);
...
export function validateAnnotatedTextEntityActions(entities: Iterable<any>) {
  ...
  for (const owner of byName.values()) for (const [fieldName, descriptor] of Object.entries(owner.fields ?? {})) {
    ...
    for (const ann of (descriptor as any).annotations ?? []) for (const action of ann.actions ?? []) {
      if (action.kind !== 'annotationEntityAction') continue;
