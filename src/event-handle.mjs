...
const LIFECYCLE_TYPES = new Set([EventKind.created, EventKind.updated, EventKind.removed]);
...
export function created(entity) {
  ...
  return freezeHandle({ entity, kind: EventKind.created, type: `${entity}.created` });
}

export function updated(entity) {
  assertName('event entity', entity);
  return freezeHandle({ entity, kind: EventKind.updated, type: `${entity}.updated` });
}

export function removed(entity) {
  assertName('event entity', entity);
  return freezeHandle({ entity, kind: EventKind.removed, type: `${entity}.removed` });
}

export function fieldSet(entity, field) {
  assertName('event entity', entity);
  assertName('event field', field);
  return freezeHandle({ entity, kind: EventKind.fieldSet, field, type: `${entity}.${field}.set` });
}

export function native(entity, field, nativeName) {
  assertName('event entity', entity);
  assertName('event field', field);
