...
import {
  ...
  type SearchPluginHealth,
  type SearchPluginRegistry,
  type SearchPluginState,
  type SearchSourceReader,
} from './search-plugin.ts';
import type {
  SearchStalenessAffected,
  SearchStalenessBridge,
  SearchStalenessRecord,
...
export type SearchShadowPlugin = SearchPlugin & SearchShadowCapabilities;

export function hasShadowCapabilities(plugin: SearchPlugin): plugin is SearchShadowPlugin {
  return (
...
export interface SearchParityComparison {
  ...
  // null when the plugin's indexCensus provides no digest (count parity only).
...
export interface SearchParityReport {
  ...
  readonly at: string;
  readonly source: SearchCensus;
  readonly index: SearchCensus;
  readonly comparisons: readonly SearchParityComparison[];
  readonly matches: boolean;
}

export interface SearchReconcileFailure {
...
export interface SearchReconcileBatchSummary {
  readonly processed: number;
  readonly reconciled: number;
  readonly rebuilt: number;
  readonly retained: number;
  // records still pending after this bounded batch (so a scheduler can loop).
  readonly remaining: number;
  readonly failures: readonly SearchReconcileFailure[];
}

export interface SearchShadowRebuildOutcome {
  readonly ok: boolean;
  readonly pluginId: string;
  readonly activated: boolean;
  readonly retired: boolean;
  readonly retryable: boolean;
  readonly fenceAtStart: number;
  readonly generation: number;
  readonly fence: number;
  readonly state: SearchPluginState;
  readonly parity: SearchParityReport | null;
  readonly lastError: string | null;
}

export interface SearchReconcileOptions {
  readonly registry: SearchPluginRegistry;
  readonly staleness: SearchStalenessBridge;
  // The engaged ledger handle — the SAME handle the bridge is engaged with
...
export interface SearchReconcileEngine {
  ...
  parity(id: string): SearchParityReport;
  healthOf(id: string): SearchPluginHealth & { readonly parity: SearchParityReport | null };
}
...
export function censusOfRows(rows: readonly Record<string, unknown>[]): SearchEntityCensus {
...
export function computeSourceCensus(reader: SearchSourceReader): SearchCensus {
  const census: Record<string, SearchEntityCensus> = {};
...
export function censusEqual(a: SearchCensus, b: SearchCensus): boolean {
...
function parseAffected(affected: string, context: string): readonly SearchStalenessAffected[] {
  try {
    const parsed = JSON.parse(affected) as unknown;
    if (!Array.isArray(parsed)) throw new Error('not an array');
    const out: SearchStalenessAffected[] = [];
...
export function createSearchReconcileEngine(options: SearchReconcileOptions): SearchReconcileEngine {
  ...
  function ledgerRow(sourceResource: string, sourceKey: string): LedgerRow | null {
    ...
    ).get(sourceResource, sourceKey) as Pick<LedgerRow, 'kind' | 'priority' | 'affectedJson' | 'payload' | 'changedAt'> | undefined;
  ...
  async function processRecord(record: SearchStalenessRecord): Promise<'reconcile' | 'rebuild' | null> {
    ...
    if (row.kind === 'source-change') {
      ...
      try {
        const parsed = JSON.parse(row.payload) as { change?: unknown } | null;
  ...
  function deleteIfUnchanged(record: SearchStalenessRecord, processed: LedgerRow): void {
  ...
  async function reconcileBatches(
    callOptions: { readonly batchSize?: number } = {},
  ): Promise<SearchReconcileBatchSummary> {
    ...
    let rebuilt = 0;
    let retained = 0;
    const failures: SearchReconcileFailure[] = [];
  ...
  function contextOf(id: string, generation: number, fence: number): SearchPluginContext {
  ...
  function parity(id: string): SearchParityReport {
    const plugin = registry.get(id);
    if (plugin === undefined) {
      throw new Error(`search plugin '${id}' is not registered with this registry`);
    }
  ...
  function healthOf(id: string): SearchPluginHealth & { readonly parity: SearchParityReport | null } {
    let report: SearchParityReport | null = null;
    try {
      report = parity(id);
    } catch {
  ...
  function canShadow(id: string): boolean {
    const plugin = registry.get(id);
  ...
  async function safeAbort(plugin: SearchShadowPlugin, id: string): Promise<void> {
    try {
      const state = registry.stateOf(id);
  ...
  async function rollbackActivation(plugin: SearchShadowPlugin, id: string): Promise<void> {
  ...
  function failedOutcome(
    ...
    error: string,
    retryable: boolean,
    parityReport: SearchParityReport | null,
    fenceAtStart: number | null,
    before: SearchPluginHealth | null,
  ): SearchShadowRebuildOutcome {
  ...
  async function rebuildShadow(id: string): Promise<SearchShadowRebuildOutcome> {
    const plugin = registry.get(id);
    if (plugin === undefined) {
