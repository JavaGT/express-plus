    ...
    for (const annotationDeclaration of annotationDeclarations) {
      for (const [actionName, action] of Object.entries(annotationDeclaration.actions ?? {}) as Array<[string, any]>) {
        if (action.kind === 'annotationAction') {
          ...
          Object.defineProperties(domainHandler, { inTransaction: { value: true }, batchForbidden: { value: true }, dedupeReceiptMatches: { value: (receipt: any, request: any) => receipt.actionType === actionType && receipt.actionData === JSON.stringify(request.payload) } });
        ...
        const threadHandler = async ({ payload, db, scope, principal, actionId }: any) => {
          ...
          for (const [fieldKey, fieldDescriptor] of Object.entries(annotationDeclaration.fields ?? {})) {
           ...
           const plan = planTextRangeApply({ documentId: payload.id, structureVersion: state.structure_version, family, annotation: { id: annotationId, family: annotationDeclaration.annotationName, fields: annotationFields } as any, from: { offset: from, affinity: 'right' }, to: { offset: to, affinity: 'left' }, ranges, actorId: principal?.id ?? '' });
          const handle = eventHandles.native(name, fieldName, 'operated');
        ...
        Object.defineProperties(threadHandler, { inTransaction: { value: true }, batchForbidden: { value: true }, dedupeReceiptMatches: { value: (receipt: any, request: any) => receipt.actionType === actionType && receipt.actionData === JSON.stringify(request.payload) } });
