...
import { applyTextOperation, compactTextFamilyCheckpoint, restoreTextFamilySerialized, textFamilyBasis } from '../annotated-text-continuous.mjs';
  ...
  for (const [fieldName, descriptor] of Object.entries(fields)                        ) {
    ...
    const r4Handler = ({ payload, db, scope, principal }     ) => {
      ...
      if (anySplit) afterRevision = state.structure_version + 1;
       ...
       try { resolvePositionToEndpoint(currentFamily, selectedBlockId, 0, basisFrontier, 'right'); } catch (error     ) {
         throw new ValidationError(`${name}.${fieldName}.operation ${error.message}`);
       }
