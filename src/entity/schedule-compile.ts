...
import { compileReadScope } from '../scope-sql.ts';
import { updated } from '../event-handle.ts';
import { self } from '../effect-runtime.ts';
import { schedule as scheduleConstructors } from '../schedule.ts';
...
interface StateFieldDescriptor {
  kind?: string;
  type?: string;
  effects?: Record<string, EffectDescriptor | undefined>;
...
type Fields = Record<string, StateFieldDescriptor>;
...
export function stateEffectEntries(entityName: string, fieldName: string, descriptor: StateFieldDescriptor) {
  const declared = descriptor.effects;
  if (declared == null || Object.keys(declared).length === 0) return [];

  const legalTransitions = new Map<string, { from: string; to: string }>();
  ...
  return Object.entries(declared).map(([key, declaredEffect]) => {
    ...
    const mutate = Object.hasOwn(declaredEffect ?? {}, 'mutate') ? (declaredEffect as EffectDescriptor).mutate : self;
...
export function assertSqlIdentifier(kind: string, value: unknown) {
...
export function autoStateScheduleTrigger(entityName: string, fieldName: string, descriptor: StateFieldDescriptor, fields: Fields) {
...
interface DeclaredScheduleTrigger {
  kind?: string;
  field?: unknown;
  when?: unknown;
  while?: unknown;
  ...
  hertz?: number;
  intervalMs?: number;
  autoState?: { fieldName?: string; when?: string };
...
export function validateScheduleTrigger({ name, verbName, trigger, fields, registry }: {
  name: string;
  verbName: string;
  trigger: DeclaredScheduleTrigger;
  fields: Fields;
  registry: unknown;
}) {
  ...
  let whileSql: string | undefined;
  let whileParams: Record<string, unknown> | undefined;
  let whileAst: unknown;
  ...
  if (trigger.autoState) {
    ...
    const compiled = compileReadScope(trigger.while as (ctx: { is: unknown; fields: unknown }) => unknown, {
      fields,
