...
export type BlobRetentionPolicyName =
  | 'abandoned-upload'
  | 'replaced-generation'
  | 'deleted-file'
  | 'privacy-erasure'
...
export const BLOB_RETENTION_POLICY_NAMES: readonly BlobRetentionPolicyName[] = Object.freeze([
  'abandoned-upload',
  'replaced-generation',
  'deleted-file',
  'privacy-erasure',
...
const POLICY_KEY: Record<BlobRetentionPolicyName, keyof BlobRetentionPolicies> = {
...
export interface BlobRetentionPolicies {
...
export const blobRetentionDefaults: Readonly<BlobRetentionPolicies> = Object.freeze({
...
export function validateBlobRetentionPolicies(
  policies?: Readonly<Partial<BlobRetentionPolicies>>,
): Readonly<BlobRetentionPolicies> {
  ...
  for (const key of Object.keys(policies) as (keyof BlobRetentionPolicies)[]) {
 ...
 */
export function retentionMs(policies: Readonly<BlobRetentionPolicies>, name: BlobRetentionPolicyName): number {
  const value = policies[POLICY_KEY[name]];
