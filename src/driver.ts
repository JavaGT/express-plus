...
async function sqliteReadSnapshotTxn(db: DbHandle, fn: () => unknown): Promise<unknown> {
  db.exec('BEGIN');
  try {
    return await fn();
  } finally {
...
export async function readSnapshotTxn(db: DbHandle, fn: () => unknown): Promise<unknown> {
...
export const CONNECTION_PRAGMA_SQL: readonly string[] = Object.freeze([
...
export function applyConnectionPragmas(db: { exec(sql: string): unknown }): void {
...
// (not `this`).
export function attachDriverHelpers(dbOrDriver: DbHandle): DbHandle {
