...
export class ReadMirrorError extends Error {
  ...
  readonly kind: ReadMirrorStatementKind;
  constructor(kind: Exclude<ReadMirrorStatementKind, 'read'>, detail: string) {
...
function scanStatement(sql: string): { words: string[]; statements: number } {
  const words: string[] = [];
  let statements = 1;
  let depth = 0;
  let index = 0;
...
function pragmaNameOf(sql: string): string | null {
...
function isPragmaAssignment(sql: string): boolean {
...
// refused fail-closed.
export function classifyReadMirrorStatement(sql: string): ReadMirrorStatementKind {
  const { words } = scanStatement(sql);
  let sawRead = false;
...
function refuse(kind: Exclude<ReadMirrorStatementKind, 'read'>, sql: string): never {
