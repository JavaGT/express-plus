...
export function readRevision(db: DbHandle, resourceKey: string): number {
  const row = prepareCached(
    db,
    'SELECT revision FROM _LiveRevision WHERE resourceKey = :resourceKey',
  ).get({ resourceKey });
  const revision = row?.revision as number | undefined;
...
export function bumpRevision(db: DbHandle, resourceKey: string): number {
  const row = prepareCached(
    db,
...
export function expectedRevisionConflict(resourceKey: string, expected: unknown, actual: unknown): Error {
...
export function guardExpectedRevision(db: DbHandle, resourceKey: string, expectedRevision: unknown) {
