...
async function measureSnapshot(db, BenchDoc, fixture, principal, visible) {
  ...
  if (PROFILE_PHASES) {
    ...
    phases.push({ phase: 'JSON.parse', durationMs: parseDuration, details: { bytes: serializedBytes } });
    phases.push({ phase: 'materializer (excl parse)', durationMs: validationDuration - parseDuration, details: { bytes: serializedBytes } });
...
async function main() {
  ...
  if (memoryTrail.length) {
    const rssPeakGc = Math.max(...memoryTrail.map((entry) => entry.rssAfterGc));
    report.memoryProbe = {
      perRecipientRssMiB: memoryTrail.map((entry) => Number(((entry.rssAfter - entry.rssBefore) / (1024 * 1024)).toFixed(2))),
      perRecipientRssAfterGcMiB: memoryTrail.map((entry) => Number(((entry.rssAfterGc - memoryTrail[0].rssBefore) / (1024 * 1024)).toFixed(2))),
      peakRssDeltaAfterForcedGcMiB: Number(((rssPeakGc - rssBefore) / (1024 * 1024)).toFixed(2)),
      heapUsedAfterMiB: memoryTrail.map((entry) => Number((entry.heapUsedAfter / (1024 * 1024)).toFixed(2))),
      heapUsedAfterGcMiB: memoryTrail.map((entry) => Number((entry.heapUsedAfterGc / (1024 * 1024)).toFixed(2))),
      externalAfterMiB: memoryTrail.map((entry) => Number((entry.externalAfter / (1024 * 1024)).toFixed(2))),
      arrayBuffersAfterMiB: memoryTrail.map((entry) => Number((entry.arrayBuffersAfter / (1024 * 1024)).toFixed(2))),
    };
  }
