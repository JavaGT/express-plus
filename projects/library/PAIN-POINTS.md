...
| 7 | No scheduler primitive for time-based transitions (overdue) | **STILL-OPEN / NEW-ANGLE** | `state.auto` exists (RESOLVED the existence question) but only supports fixed duration from state-entry. Conditional-by-field-value deadlines (Blocked #2) and entity `tick` (Phase 2 step 9) are not shown. |
