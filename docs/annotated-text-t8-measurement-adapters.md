...
Adapters receive frozen, opaque JSON payloads (packed in the `measurement` payload column). The `validate`/`edit`/`partition`/`combine` signatures and return contracts are reserved for T4 atomic split/merge orchestration. No Scope or action integration is planned at T8.
