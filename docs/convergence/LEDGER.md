...
| S scope migration | `S-scope-migration.md` | **gate ready — owner decision** | — | — | Both conditions met: Scope refactor v2 complete (wave-3 merged, 1933/1934, clean tree). W-packets at parity (W1-W5 done, 1576/1576). Council c04 (Opus + GPT, cross-evaluated) converged: station A is transport swap only (SSE→WebSocket), no re-key, no seq change. Station A blocked on owner gate-opening decision. |
