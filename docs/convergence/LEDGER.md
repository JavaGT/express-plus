...
| W5 client engine parity | `W5-client-engine-parity.md` | **done (1 W5 filing remains)** | `convergence/W5-scope-subscription` (slices 1+2), main (c03 filing, integration proof) | 1612/1612/0 | Slice 1+2: subscribeScope() + scope-keyed fan-out. c03 filing: LiveChannel.onConnectionChange emitter (4 transition points). Integration proof: createLiveStore + overlay + canUndoField. 1 W5 filing remains: overlay-status API (station C) — client-overlays.ts in Scope's `src/lib/wb-scope/`. |
| S scope migration | `S-scope-migration.md` | **building (station C steps 1-3/6)** | `phase-S/station-A` (Scope repo), `main` (Scope repo) | 1970 (Scope), 0 check errors | Station A: WS transport adapter (3 surfaces). Station B: Source/Note/ExternalRef CRUD → workbench entities. Station C Steps 1-3: contract tests locked (store-adapter, 19 tests), client-overlays.ts adapter (7 tests), wired into store export. Remaining Station C: Steps 4-6 (consumer migration, upstream review, configurable transport). S0 joint recommendation: station A is transport swap (SSE→WS). | |
